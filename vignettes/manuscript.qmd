...
However, these valuable datasets, showing how frequently individual clinical code were used, are difficult to access and analyse in their current format, hindering their application in research and service evaluation.

We developed *OpenCodeCounts*, an interactive online tool with an accompanying R package that help users explore and analyse England's primary and secondary care clinical coding data.
...
To minimise package size, we did not incorporate breakdowns by age and gender.
While these variables are informative, they increase the dataset size, making the package less efficient for general use.
...
tribble(
  ...
  gt() |>
  tab_spanner(label = "Dataset", columns = c(snomed, icd10, opcs4)) |>
  cols_label(
    column = "Column",
    example = "Example",
    snomed = md("*snomed_usage*"),
    icd10 = md("*icd10_usage*"),
    opcs4 = md("*opcs4_usage*")
  ) |>
  cols_align(align = "center", columns = c(snomed, icd10, opcs4)) |>
  tab_style(
...
The current version of the package consists of two broad components: (1) three standardised datasets containing code usage counts for different clinical coding systems, and (2) a function to import specific codelists from [OpenCodelists.org](https://www.opencodelists.org/) into R and the accompanying interactive tool (*Shiny* app).
...
tribble(
  ...
  "`snomed_usage`"    , "SNOMED CT code usage in primary care starting from 2011"                             , "Datasets"  ,
  "`icd10_usage`"     , "ICD-10 code usage in secondary care admissions in acute hospitals starting from 2012" , "Datasets"  ,
  "`opcs4_usage`"     , "OPCS-4 code usage in secondary care admissions in acute hospitals starting from 2012" , "Datasets"  ,
...
The *opencodecounts* R package is freely available from [Github](https://github.com/bennettoxford/opencodecounts) and can be installed in R using `remotes::install_github("bennettoxford/opencodecounts")` and loaded using `library(opencodecounts)`.
...
The online tool allows users to select a custom group of codes, download the data and visualise trends.
For bespoke analysis or visualisation, users can also work directly within R with access to the underlying data.
This means users can produce visualisations and analysis for any of their chosen codes, across any available years and in a preferred graphical style.
The `get_codelist()` function allows users to access codelists from [OpenCodelists.org](https://www.opencodelists.org/).
The following examples demonstrate how to extract semantic tags from SNOMED CT codes (see @fig-semantic-tags) and how to find the three most used ICD-10 codes from a selected codelist and visualise their trends over time (see @fig-top-icd10).

#### Extracting semantic tags from SNOMED CT codes

Using the codelist for [Depression screening codes](https://www.opencodelists.org/codelist/nhsd-primary-care-domain-refsets/depscrn_cod/20250627/), we grouped code usage by semantic tag to explore how different tags were used over time.
@fig-semantic-tags shows the yearly code usage across semantic tags such as "Procedure", "Observable entity", and "Finding", highlighting an increased coding of observable entities (scores of depression screenings) in recent years.

```{r}
#| label: fig-semantic-tags
#| echo: true
#| fig-cap: "Trends over time in depression screening code usage grouped by semantic tag."
#| fig-width: 9
#| fig-height: 5.5
# Load packages
library(dplyr)
...
# Get depression screening codes from OpenCodelists.org
depscrn_codelist <- get_codelist(
  "https://www.opencodelists.org/codelist/nhsd-primary-care-domain-refsets/depscrn_cod/20250627/"
)

# Group data from codelist by semantic tags and calculate yearly sums
depscrn_cod_usage <- snomed_usage |>
  filter(snomed_code %in% depscrn_codelist$code) |>
  mutate(
  ...
  group_by(end_date, semantic_tag) |>
  summarise(total_usage = sum(usage), .groups = "drop")

# Visualise the trends over time for semantic tags from codelist
depscrn_cod_usage |>
  ggplot(aes(y = total_usage, x = end_date,
             colour = semantic_tag, shape = semantic_tag)) +
  geom_line(alpha = 0.2) +
  geom_point(size = 2) +
  labs(x = "End date of yearly interval", y = "Usage count",
       colour = NULL, shape = NULL) +
  scale_x_date(breaks = unique(depscrn_cod_usage$end_date),
               date_labels = "%b\n%Y") +
...
#### Identifying the most frequently used ICD-10 codes

Using the [ICD-10 Chapter XIX](https://www.opencodelists.org/codelist/opensafely/icd-10-chapter-xix/4dce479b/) codelist (injury, poisoning, and certain other consequences of external causes), we identified the three most frequently used codes since 2014 and plotted their usage over time.
The results highlight a relatively stable pattern for the most used code *Fracture of neck of femur* (S720), an increase in usage for *Poisoning: 4-Aminophenol derivatives* (T391) up until March 2020 and a decrease in code usage of *Superficial injury of other parts of head* (S008) from March 2022 onwards, see @fig-top-icd10.

```{r}
#| label: fig-top-icd10
#| echo: true
#| fig-cap: "Trends over time in usage of the three most common ICD-10 Chapter XIX codes."
#| fig-width: 9
#| fig-height: 5.5
# Load packages
library(dplyr)
...
# Get codelist from OpenCodelists.org
icd10_xix_codelist <- get_codelist(
  "https://www.opencodelists.org/codelist/opensafely/icd-10-chapter-xix/4dce479b/"
)
...
# Visualise trends over time for the 3 selected codes
df_icd10_xix |>
  ...
  scale_x_date(breaks = unique(df_icd10_xix$end_date), date_labels = "%b\n%Y") +
...
NHS publications also provide breakdowns by sex and age, which could be informative, but come at the cost of increasing the package size and reducing processing speeds.
...
BMK is also employed by NHS England working on medicines policy and clinical lead for primary care medicines data.
Other authors declare no competing interests.

## Authors contributions

Conceptualization: AAT, RH, BMK, HJC, LF, MW.\
Data curation: AAT, VK, LO, KO, LF, MW.\
Formal Analysis: AAT, YB, MW.\
Funding acquisition: RH, AS, AG, WH, BMK, HJC.\
Methodology: AAT, RH, BMK, HJC, LF, MW.\
Codelists (subset of methodology): RH, YB.\
Project administration: AAT, RH, MW.\
Software: AAT, RH, VK, LO, KO, LF, MW.\
Supervision: HJC, MW\
Validation: AAT, RH, YB, VK, AS, WH, BMK, HJC, MW\
Visualization: AAT, RH, MW\
Writing -- original draft: AAT, YB, MW\
Writing -- review & editing: AAT, RH, YB, VK, LO, KO, JM, AS, AG, WH, BMK, HJC, LF, MW
