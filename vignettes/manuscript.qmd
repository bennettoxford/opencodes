...
To keep the main datasets small and efficient for general use, these breakdowns are held in separate datasets.
No equivalent breakdowns are published for the primary care SNOMED CT data.
...
tab_datasets <- tribble(
  ~column           , ~example                      , ~snomed , ~icd10_opcs4 , ~icd10_opcs4_bd ,
  "start_date"      , "Date: YYYY-MM-DD"            , "+"     , "+"          , "+"             ,
  "end_date"        , "Date: YYYY-MM-DD"            , "+"     , "+"          , "+"             ,
  "code *"          , "String: 1234 or A000"        , "+"     , "+"          , "+"             ,
  "description"     , "String: Description of code" , "+"     , "+"          , "+"             ,
  "usage"           , "Numeric: 100000"             , "+"     , "+"          , "+"             ,
  "breakdown"       , "String: female or age_45_49" , "–"  , "–"       , "+"             ,
  ...
  tab_spanner(
    label = "Dataset",
    columns = c(snomed, icd10_opcs4, icd10_opcs4_bd)
  ) |>
  cols_label(
    column = "Column",
    example = "Example",
    snomed = md("*snomed_usage*"),
    icd10_opcs4 = md("*icd10_usage*<br>*opcs4_usage*"),
    icd10_opcs4_bd = md("*icd10_usage_breakdowns*<br>*opcs4_usage_breakdowns*")
  ) |>
  cols_align(
    align = "center",
    columns = c(snomed, icd10_opcs4, icd10_opcs4_bd)
...
if (knitr::is_html_output()) {
  ...
  gtsave(tab_datasets, "img/manuscript_table1.png")
  knitr::include_graphics("img/manuscript_table1.png")
}
```
...
The current version of the package consists of two broad components: (1) five standardised datasets containing code usage counts for different clinical coding systems, including breakdowns by sex and age group for the hospital datasets, and (2) a function to import specific codelists from [OpenCodelists.org](https://www.opencodelists.org/) into R and the accompanying interactive tool (*Shiny* app).
...
tab_functions <- tribble(
  ...
  "`icd10_usage_breakdowns`"  , "ICD-10 code usage with breakdowns by diagnosis position, sex and age group starting from 2012"         , "Datasets"  ,
  "`opcs4_usage_breakdowns`"  , "OPCS-4 code usage with breakdowns by procedure position, sex and age group starting from 2012"         , "Datasets"  ,
...
if (knitr::is_html_output()) {
  ...
  gtsave(tab_functions, "img/manuscript_table2.png")
  knitr::include_graphics("img/manuscript_table2.png")
}
```
...
The example combines coding activity from primary care (SNOMED CT) and hospital admitted care (OPCS-4) in one analysis (see @fig-biopsy-mri-codes) and uses the demographic breakdowns to examine age-specific trends in hospital procedures (see @fig-biopsy-age).
...
The breakdown datasets allow these trends to be examined by sex and age group without any patient-level data.
...
df_biopsy_age <- opcs4_usage_breakdowns |>
  filter(opcs4_code %in% codes_opcs4_biopsy) |>
...
The breakdowns by sex and age published for the hospital data are provided as separate datasets, so that the main datasets remain small and fast to work with.
No equivalent breakdowns are published for the primary care SNOMED CT data.
